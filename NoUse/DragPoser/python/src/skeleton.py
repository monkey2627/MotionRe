...
def create_pooling_list(parents, add_displacement=True):
    # vertices degree
    ...
    if add_displacement:
        pooling_list.append([])
        for i in range(len(parents)):  # all joints use displacement
...
class SkeletonPool(nn.Module):
    def __init__(self, pooling_list, old_parents, channels_per_edge, device):
        super().__init__()
        ...
        self.description = "SkeletonPool(in_joints_num={}, out_joints_num={})".format(
            len(old_parents), len(pooling_list)
        )

        # rows: new joints, columns: old joints
        num_new_joints = len(pooling_list)
        num_old_joints = len(old_parents)
        self.weight = torch.zeros(
            num_new_joints * channels_per_edge,
            num_old_joints * channels_per_edge,
        ).to(self.device)
...
class SkeletonUnpool(nn.Module):
    def __init__(self, pooling_list, channels_per_edge, device):
        ...
        for merged in pooling_list:
            for j in merged:
                out_joints.add(j)
        output_joints_num = len(out_joints) + 1  # add displacement
...
def find_neighbor(parents, max_dist, add_displacement=True):
    ...
    if add_displacement:
        ...
        # displacement has same neighbors as the root joint
