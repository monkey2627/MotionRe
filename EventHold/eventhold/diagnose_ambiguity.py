...
def main():
    ...
    rows, coverage, hashes, all_pairs = [], [], {}, []
    for record in sources:
        ...
        for setting, choices in selected:
            # Nearest-neighbor retrieval alone can hide other compatible poses.
            # Score ALL unique measurement-qualified pairs as a separate audit.
            r0, a0 = distances[0]
            qualified = ((r0.max(axis=-1) <= setting['max_sensor_rotation_deg'])
                         & (a0.max(axis=-1) <= setting['max_sensor_acc_difference_m_s2'])
                         & (np.abs(frames[:, None]-frames[None, :]) >= cfg['minimum_pair_separation_frames']))
            left, right = np.where(np.triu(qualified, k=1))
            for i, j in zip(left, right):
                t, u = int(frames[i]), int(frames[j])
                if not (gt_valid[t] and gt_valid[u]):
                    continue
                for region, ids in cfg['regions'].items():
                    error = float(angle_error(gt[t, ids], gt[u, ids]).mean())
                    all_pairs.append({'source':str(source), 'subject':source.parent.name,
    ...
    if all_pairs:
        write_csv(args.output/'all_qualified_pairs.csv', all_pairs, list(all_pairs[0]))
    ...
    for setting in cfg['settings']:
        for region in cfg['regions']:
            group=[r for r in all_pairs if r['setting']==setting['name'] and r['region']==region]
            all_pair_summary.append({'setting':setting['name'],'region':region,
                                     'unique_pairs':len(group),
                                     'large_gap_pairs':sum(r['large_pose_gap'] for r in group),
                                     'max_gap_deg':max((r['reference_pose_gap_deg'] for r in group),default=None),
                                     'warning':'pair_count_not_independent_sample_size'})
    summary={'status':'exploratory_same_sequence_retrospective_retrieval_not_deployed_model',
             ...
             'primary_large_gap_unique_pairs_scope':'current_nearest_neighbor_only',
             'all_qualified_pair_summary':all_pair_summary,
