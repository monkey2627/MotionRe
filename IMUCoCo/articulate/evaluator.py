...
class FullMotionEvaluator(BasePoseEvaluator):
    ...
    def __call__(self, pose_p, pose_t, shape_p=None, shape_t=None, tran_p=None, tran_t=None):
        ...
        return torch.tensor([[je.mean(),   je.std(dim=0).mean()],   # joint position error
                             [ve.mean(),   ve.std(dim=0).mean()],   # vertex position error
                             [lae.mean(),  lae.std(dim=0).mean()],  # local angle error
                             [gae.mean(),  gae.std(dim=0).mean()],  # global angle error
                             [jkp.mean(),  jkp.std(dim=0).mean()],  # predicted jerk
                             [jkt.mean(),  jkt.std(dim=0).mean()],  # true jerk
                             [te.mean(),   te.std(dim=0).mean()],   # translation error
