...
namespace RootMotion.FinalIK {
	...
	public class IKSolverLeg : IKSolver {
		...
		/// <summary>
		/// A direct target for an individual leg joint.
		/// </summary>
		[System.Serializable]
		public class JointTarget {
			[Tooltip("Optional target Transform for this joint. If assigned, IKPosition and IKRotation will be overwritten by this Transform each update.")]
			public Transform target;
			[Tooltip("Positional weight of this joint target.")]
			[Range(0f, 1f)] public float positionWeight;
			[Tooltip("Rotational weight of this joint target.")]
			[Range(0f, 1f)] public float rotationWeight;

			/// <summary>
			/// Target position of this joint. Will be overwritten if target is assigned.
			/// </summary>
			[System.NonSerialized][HideInInspector] public Vector3 IKPosition;

			/// <summary>
			/// Target rotation of this joint. Will be overwritten if target is assigned.
			/// </summary>
			[System.NonSerialized][HideInInspector] public Quaternion IKRotation = Quaternion.identity;

			public void SetDefault(Transform transform) {
				if (transform == null) return;

				IKPosition = transform.position;
				IKRotation = transform.rotation;
			}
			...
			public bool IsUsed() {
				return positionWeight > 0f || rotationWeight > 0f;
			}
		}
		...
		public JointTarget pelvisTarget = new JointTarget();
		public JointTarget thighTarget = new JointTarget();
		public JointTarget calfTarget = new JointTarget();
		...
		protected override void OnInitiate() {
			...
			pelvisTarget.SetDefault(pelvis.transform);
			thighTarget.SetDefault(thigh.transform);
			calfTarget.SetDefault(calf.transform);
		...
		private void Read() {
			...
			ApplyJointTarget(pelvisTarget, 1, 5);
			ApplyJointTarget(thighTarget, 2, 5);
			ApplyJointTarget(calfTarget, 3, 5);
		...
		private void Write() {
			...
			if (pelvisTarget.IsUsed()) {
				pelvis.transform.position = positions[1];
				pelvis.transform.rotation = rotations[1];
			}

			if (thighTarget.positionWeight > 0f) thigh.transform.position = positions[2];
		...
		private void ApplyJointTarget(JointTarget jointTarget, int index, int lastIndex) {
			jointTarget.ReadTarget();

			float positionWeight = Mathf.Clamp01(jointTarget.positionWeight);
			if (positionWeight > 0f) {
				Vector3 targetPosition = Vector3.Lerp(positions[index], jointTarget.IKPosition, positionWeight);
				Vector3 delta = targetPosition - positions[index];
			...
			float rotationWeight = Mathf.Clamp01(jointTarget.rotationWeight);
			if (rotationWeight <= 0f) return;

			Quaternion targetRotation = Quaternion.Slerp(rotations[index], jointTarget.IKRotation, rotationWeight);
			Quaternion deltaRotation = targetRotation * Quaternion.Inverse(rotations[index]);
			Vector3 pivot = positions[index];

			for (int i = index; i <= lastIndex; i++) {
				if (i > index) {
					positions[i] = pivot + deltaRotation * (positions[i] - pivot);
				}
