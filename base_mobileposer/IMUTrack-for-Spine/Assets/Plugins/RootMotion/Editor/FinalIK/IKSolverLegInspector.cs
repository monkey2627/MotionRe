...
namespace RootMotion.FinalIK {
	...
	public class IKSolverLegInspector: IKSolverInspector {
		...
		public static void AddInspector(SerializedProperty prop, bool editHierarchy, bool showReferences) {
			...
			// Joint targets
			AddJointTarget(prop.FindPropertyRelative("pelvisTarget"), new GUIContent("Pelvis Target", "Optional direct position/rotation target for the pelvis/hip root joint."));
			AddJointTarget(prop.FindPropertyRelative("thighTarget"), new GUIContent("Thigh Target", "Optional direct position/rotation target for the upper leg root joint."));
			AddJointTarget(prop.FindPropertyRelative("calfTarget"), new GUIContent("Calf/Knee Target", "Optional direct position/rotation target for the knee/lower leg joint."));
		...
		private static void AddJointTarget(SerializedProperty prop, GUIContent label) {
			EditorGUILayout.Space();
			EditorGUILayout.LabelField(label, EditorStyles.boldLabel);
			EditorGUILayout.PropertyField(prop.FindPropertyRelative("target"), new GUIContent("Target", label.tooltip));
			EditorGUILayout.PropertyField(prop.FindPropertyRelative("positionWeight"), new GUIContent("Position Weight", "Solver weight for smooth blending to this joint target position."));
			EditorGUILayout.PropertyField(prop.FindPropertyRelative("rotationWeight"), new GUIContent("Rotation Weight", "Solver weight for smooth blending to this joint target rotation."));
		}
		...
		public static void AddScene(IKSolverLeg solver, Color color, bool modifiable) {
			...
			AddJointTargetScene(solver.pelvisTarget, solver.pelvis.transform, color, modifiable);
			AddJointTargetScene(solver.thighTarget, solver.thigh.transform, color, modifiable);
			AddJointTargetScene(solver.calfTarget, solver.calf.transform, color, modifiable);
			
			Handles.color = Color.white;
			GUI.color = Color.white;
		}

		private static void AddJointTargetScene(IKSolverLeg.JointTarget jointTarget, Transform joint, Color color, bool modifiable) {
			if (!Application.isPlaying) return;
			if (jointTarget == null || joint == null) return;
			if (!jointTarget.IsUsed()) return;

			jointTarget.ReadTarget();

			if (modifiable && jointTarget.target == null) {
				Handles.color = color;
				Inspector.CubeCap(0, jointTarget.IKPosition, jointTarget.IKRotation, GetHandleSize(jointTarget.IKPosition));

				switch(Tools.current) {
				case Tool.Move:
					if (jointTarget.positionWeight > 0f) jointTarget.IKPosition = Handles.PositionHandle(jointTarget.IKPosition, Quaternion.identity);
					break;
				case Tool.Rotate:
					if (jointTarget.rotationWeight > 0f) jointTarget.IKRotation = Handles.RotationHandle(jointTarget.IKRotation, jointTarget.IKPosition);
					break;
				}
			}

			float weight = Mathf.Max(jointTarget.positionWeight, jointTarget.rotationWeight);
			if (weight <= 0f) return;

			Handles.color = new Color(color.r, color.g, color.b, color.a * weight);
			Handles.DrawLine(joint.position, jointTarget.IKPosition);
		}
